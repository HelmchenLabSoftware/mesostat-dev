...
def paired_test(f, x, y, nSample):
    ...
    fTrue = f(x, y)
    fTestArr = resampling.resample_dyad_individual(f, x, y, nSample=nSample, method="permutation")
...
def difference_test(f, x, y, nSample):
    assert x.shape == y.shape
    fTrue = f(x, y)
    fTestArr = resampling.resample_dyad_union(f, x, y, nSample, method="resample")
    return percentile_twosided(fTrue, fTestArr)
