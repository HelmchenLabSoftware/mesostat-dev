...
class AR1D:
    ...
    # FIXME: we have swapped shape to [nTrial, nTime]. Adjust all uses
    def data2xy(self, data):
        nHist = self.nHist

        sweepTime = lambda d : np.array([d[iTime:iTime + nHist] for iTime in range(len(d) - nHist)])

        y = np.hstack([dataTrial[nHist:] for dataTrial in data])
        x = np.vstack([sweepTime(dataTrial) for dataTrial in data])

        # for dataTrial in data:
        #     nTime = len(dataTrial)
        #
        #     y = dataTrial[nHist:]
        #     x = np.array([dataTrial[iTime:iTime + nHist] for iTime in range(nTime - nHist)])
